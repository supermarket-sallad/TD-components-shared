...
uniform float u_time_step, u_vorticity, u_smagorinsky, u_viscosity, u_temp_diffusion, u_temp_factor;
out vec4 fragColor;
...
void main(){
	...
	float temp_factor = smoothstep(0.0, 1.0, abs(current_state.w * u_time_step * u_temp_factor) * 1.0);
	...
	vel += force;
	float clear_press = pressure(vel_tex, uv) * 0.9999;
...
vec2 safenorm2(vec2 v){
	...
	v = normalize(v);
	v = any(isinf(v)) || any(isnan(v)) ? vec2(0) : v;
	return v;
}
...
void main()
{
    ...
    vec2 normal = safenorm2(grad);
        ...
        vec2 refl = reflect(normal, normalize(vel)); // read the docs dummy
        vel = mix(vel, refl * 0.5 * length(vel), bound); // refl * 0.9 wil fix - just blows up
        vel *= 1.0 - bound;
...
void main() {
	...
	vec2 vel = rk4advection(vel_tex, uv, u_time_step) * 2.0 * u_amp;

	p.xy += vel;
	vec2 infos = texture(vel_tex, uv).zw;
		
	P[id] = p; 
	Vel[id] = vec3(vel, 0.0);
	Temperature[id] = infos.x * u_time_step;
	Bounds[id] = clamp(1.0-infos.y,0.0,1.0);
}
